...
**Report the result clearly.** You want to see `# pass 9 / # fail 0`. If any test fails, stop and show which one — do not proceed to GitHub with failing tests.

🟡 **ASK 2** — Once green:
> "All 9 fixtures pass. Want me to (a) boot the MCP server and run the demo driver, (b) skip to GitHub setup, or (c) stop here?"
